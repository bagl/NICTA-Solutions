...
-- True
bindParser :: (a -> Parser b)
           -> Parser a
           -> Parser b
bindParser f pa = P $ \i -> case parse pa i of
  Result i' a   -> parse (f a) i'
  ErrorResult e -> ErrorResult e

-- | This is @bindParser@ with the arguments flipped.
-- It might be more helpful to use this function if you prefer this argument order.
flbindParser :: Parser a
             -> (a -> Parser b)
             -> Parser b
...
satisfy :: (Char -> Bool)
        -> Parser Char
satisfy p = character >>= \c -> bool unexpectedCharParser return (p c) c
...
-- True
natural :: Parser Int
natural = list1 digit >>= \ds -> case read ds of
  Full i -> return i
  Empty  -> failed
...
surnameParser = lift2 (:.) upper $ (++) <$> thisMany 5 lower <*> list lower
...
phoneParser = lift2 (:.) digit phoneBodyParser <* is '#'
