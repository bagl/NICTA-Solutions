...
-- hexadecimal characters. See http://json.org for the full list of
-- control characters in JSON.
--
-- /Tip:/ Use `hex`, `fromSpecialCharacter`, `between`, `is`,
--            `charTok`, `toSpecialCharacter`.
--
...
jsonNumber = P $ \i ->
  optional (ErrorResult Failed) (uncurry $ flip Result) $ readFloats i
...
jsonValue = JsonRational False <$> jsonNumber
